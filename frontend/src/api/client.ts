...
export class ApiClient {
    ...
    async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
        ...
        const isFormData = typeof FormData !== 'undefined' && body instanceof FormData;
        const payload =
            !isFormData && body !== undefined && body !== null ? JSON.stringify(body) : (body as BodyInit | null);
        ...
        if (!response.ok) {
            const errorPayload = isJSON ? (parsed as ErrorResponse) : {code: 'UNKNOWN', message: String(parsed || '')};
