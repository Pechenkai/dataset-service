...
- Метрики и здоровье: `/metrics` и `/healthz` на порте сервиса (по умолчанию `8080`). Prometheus таргеты добавлены в `deploy/monitoring/prometheus.yml`. В Compose gateway опубликован на хосте как `8081`.
...
- Демонстрационный сценарий: запрос категорий через асинхронную цепочку `GET http://localhost:8081/async/categories` → Gateway → Core → Data → Core → Gateway. Логи и метрики попадают в Loki/Prometheus. Основной Nginx-шлюз на `18088` не проксирует маршруты `/async/*`.
